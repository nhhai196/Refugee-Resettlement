...
function mttce(currMatch)
    ...
    while (~all(FamPerMatch))
        ...
        if (length(cycles) >= 1)
            ...
            [f, fpointingto, picked, Matching] = rejectionStage(Matching, G, picked);
            ...
            if (Matching(f, fpointingto) == -1)
                ...
                [~, ~, ~,  Matching] = rejectionStage(Matching, G, picked);
            end
        end
        %Matching
...
function [f, fpointingto, picked, newMatch] = rejectionStage(currMatch, G, picked)
