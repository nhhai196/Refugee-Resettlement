...
pickfam = 0; % 0: random, 1: ascending , 2: descending
order = ["random", "small-to-large", "large-to-small"];

%%flag for no algorithm
noal = 1;

%%Init
initGlobalVariablesforData(fsfile, lcfile, ewfile, compfile, dim);
%for debugging, small example
...
avgpriority = zeros(4, numL, loops);
avgfamsize = zeros(4, numL, loops);
...
% if dim == 1 
%     val = 77.75;
%     endowment = xlsread('endowment1.xlsx');
% else
%     val = 71.21;
%     endowment = xlsread('endowment3.xlsx');
% end

%endowment = zeros(329, 20);
...
for i = 1:loops
    ...
    if pickfam == 0 % random 
        ...
        % for no algo, remove this after testing
        if noal
            Matching = endowment;
        end
        %% Statistics
        for j = 1:3
            ...
            avgpriority(j,:,i) = averageMatchedPriority(temp, LocPri);
            avgfamsize(j,:,i) = averagefamsize(temp);
...
%     % Run MTTCE algorithm
    ...
    % Efficiency
    nummatched(i,j) = numberOfMatchedFamilies(temp);
    avgprefrank(i,j) = averagePreferenceRank(temp, OldFamPref);
    avgprefrankall(i,j) = averagePreferenceRankAll(temp, OldFamPref);
    cdfpref(j,:,i) = cdfPreference(temp, OldFamPref);
    cdfprefall(j,:,i) = cdfPreferenceAll(temp, OldFamPref);
    avgunfilled(i,j) = averageUnfilledQuota(temp);
    ...
    strictlybetter(i) = countBetterOff(temp, endowment);
...
mean_avgpriority = round(mean(avgpriority, 3), 3);
mean_avgfamsize = round(mean(avgfamsize, 3), 3);
% Standard deviation
...
endowtype = 'OPT';
prefname = 'type';
filename = strcat('new-no-algo-', int2str(dim),'-dim-', prefname, '-', int2str(preftype), '-', endowtype, '-', order(pickfam+1), '.xlsx' );
sheet = 1;
xlRange = 'A1';
...
% New
xlRange = 'A35';
xlswrite(filename, mean_avgpriority, sheet, xlRange);

xlRange = 'A40';
xlswrite(filename, mean_avgfamsize, sheet, xlRange);
